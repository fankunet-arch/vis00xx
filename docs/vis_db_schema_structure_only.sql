...
CREATE TABLE `vis_video_series_rel` (
  ...
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='视频与系列关联表（新）';
...
ALTER TABLE `vis_video_series_rel`
  ADD PRIMARY KEY (`video_id`,`series_id`),
  ADD KEY `idx_series_id` (`series_id`),
...
DROP VIEW IF EXISTS `vis_video_details_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`mhdlmskp2kpxguj`@`%` SQL SECURITY DEFINER VIEW `vis_video_details_view`  AS SELECT `v`.`id` AS `id`, `v`.`title` AS `title`, `v`.`platform` AS `platform`, `v`.`category` AS `category`, `c`.`category_name` AS `category_name`, `v`.`product_id` AS `product_id`, `p`.`product_name` AS `product_name`, `p`.`product_code` AS `product_code`, group_concat(distinct `s`.`id` order by `s`.`id` ASC separator ',') AS `series_ids`, group_concat(distinct `s`.`series_name` order by `s`.`series_name` ASC separator ', ') AS `series_names`, group_concat(distinct `s`.`series_code` order by `s`.`series_code` ASC separator ',') AS `series_codes`, (select min(`sub_s`.`id`) from (`vis_video_series_rel` `sub_vsr` join `vis_series` `sub_s` on((`sub_vsr`.`series_id` = `sub_s`.`id`))) where (`sub_vsr`.`video_id` = `v`.`id`)) AS `series_id`, (select `sub_s`.`series_name` from (`vis_video_series_rel` `sub_vsr` join `vis_series` `sub_s` on((`sub_vsr`.`series_id` = `sub_s`.`id`))) where (`sub_vsr`.`video_id` = `v`.`id`) order by `sub_s`.`id` limit 1) AS `series_name`, (select `sub_s`.`series_code` from (`vis_video_series_rel` `sub_vsr` join `vis_series` `sub_s` on((`sub_vsr`.`series_id` = `sub_s`.`id`))) where (`sub_vsr`.`video_id` = `v`.`id`) order by `sub_s`.`id` limit 1) AS `series_code`, `v`.`season_id` AS `season_id`, `se`.`season_name` AS `season_name`, `se`.`season_code` AS `season_code`, `v`.`r2_key` AS `r2_key`, `v`.`cover_url` AS `cover_url`, `v`.`duration` AS `duration`, `v`.`file_size` AS `file_size`, `v`.`mime_type` AS `mime_type`, `v`.`original_filename` AS `original_filename`, `v`.`status` AS `status`, `v`.`created_by` AS `created_by`, `v`.`created_at` AS `created_at`, `v`.`updated_at` AS `updated_at` FROM (((((`vis_videos` `v` left join `vis_categories` `c` on((`v`.`category` = `c`.`category_code`))) left join `vis_products` `p` on((`v`.`product_id` = `p`.`id`))) left join `vis_seasons` `se` on((`v`.`season_id` = `se`.`id`))) left join `vis_video_series_rel` `vsr` on((`v`.`id` = `vsr`.`video_id`))) left join `vis_series` `s` on((`vsr`.`series_id` = `s`.`id`))) WHERE (`v`.`status` = 'active') GROUP BY `v`.`id`, `v`.`title`, `v`.`platform`, `v`.`category`, `c`.`category_name`, `v`.`product_id`, `p`.`product_name`, `p`.`product_code`, `v`.`season_id`, `se`.`season_name`, `se`.`season_code`, `v`.`r2_key`, `v`.`cover_url`, `v`.`duration`, `v`.`file_size`, `v`.`mime_type`, `v`.`original_filename`, `v`.`status`, `v`.`created_by`, `v`.`created_at`, `v`.`updated_at` ;
...
--
-- 限制表 `vis_video_series_rel`
--
ALTER TABLE `vis_video_series_rel`
  ADD CONSTRAINT `fk_vvsr_series` FOREIGN KEY (`series_id`) REFERENCES `vis_series` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_vvsr_video` FOREIGN KEY (`video_id`) REFERENCES `vis_videos` (`id`) ON DELETE CASCADE;
COMMIT;
